<?php
/**
 * Program: electricity_history3
 * 
 * Display graph of electricity usage
 * php version 7.2.10
 * 
 * @category WebPage
 * @package  Sample
...
if (empty($_SESSION["selected_people_id"]) or check_role("RESIDENT")) {
    ...
    //$message .= " Surname: " . $_SESSION["user_surname"] . " Count of rows: " . 
    //count($rows);
}
//    var_dump($message);
//    var_dump($_SESSION);
...
<html>
  <head>
   ...
   <title>
    <?php 
    if (isset($title)) {
        echo CLIENT_NAME . " : " . htmlspecialchars($title);
    } else {
        echo CLIENT_NAME;
    }
    ?>
    ...
    <script type="text/javascript">
      ...
      function drawVisualization() {
        ...
        <?php
        $rows = query(
            "select surname, first_name, cottage from people where id = ?", 
            $_SESSION["selected_people_id"]
        );
        ...
        $rows = query(
            ...
            "(close_reading - open_reading) as `Units` " .
            "from electricity where meter = '$cottage' " .
            "and reading_date > date_add(now(), interval -2 year) " .
  ...
  <body>
    ...
    <h1><font color="white"><?php echo CLIENT_NAME; ?> : <?php echo SYSTEM_NAME ?>
        </font>
    </h1>
                    </td>
                </tr>
...
?>
            <div>
                ...
                Settlers Park Association, Horton Road, Port Alfred, South Africa
            </div>
