...
//);
$rows = query("SELECT date(create_date) as Created, count(*) as Tasks FROM `tasks` where create_date > date_add(now(), interval -30 day) group by date(create_date);");
foreach ($rows as $row) { 
    echo "['" . $row["Created"] . "', " . $row["Tasks"] . "],\r";
} 
    echo "['', 0]\n\r";
?>
        ]);
